    ...
    ui(new Ui::MainWindow)
{
    ...
    mAudioCommandReceiver = new AudioCommandReceiver();
    // соединение сигналов от команд
    connect(mAudioCommandReceiver, SIGNAL(moveForward()), this, SLOT(on_runSolveButton_clicked()));
    connect(mAudioCommandReceiver, SIGNAL(stopSolve()), this, SLOT(on_stopSolveButton_clicked()));
    connect(mAudioCommandReceiver, SIGNAL(stopMove()), this, SLOT(onStopAudioCommand_received()));
    ...
    inet_aton("192.168.1.11", &si_other.sin_addr);
...
void MainWindow::onStopAudioCommand_received()
{
    on_stopButton_clicked();
    mAudioSocket->playText("Впереди препятствие. Останавливаюсь");
}
...
void MainWindow::on_disconnectButton_clicked() // TODO
{
    ...
    mAudioCommandReceiver->Disconnect();
    ui->audioGroupBox->setEnabled(false);
}
...
void MainWindow::on_runSolveButton_clicked()
{
    ...
    mTimer->setTimePointStop(STOP_PERIOD);
    mTimer->startCount();
}
...
{
    mAudioSocket->playText(ui->audiofileInput->text());
}

void MainWindow::on_playFileButton_clicked()
{
    QString data;
    QString filename = QFileDialog::getOpenFileName(this, QString::fromUtf8("Открыть файл"), QDir::currentPath(), "Text (*.txt)");
    QFile file(filename);
    if(file.exists())
    {
